  ...
  <el-dialog title="客户添加/编辑" width="80%" :visible="showDialog"
    ...
    <el-form :form="form" :model="form" ref="form" label-width="100px" label-position="right">
      <el-row>
        <el-col :span="8">
          <el-form-item size="mini" label="客户">
            <!--<el-autocomplete placeholder="请输入客户名称,选择搜索值" :disabled="isEdit" :fetch-suggestions="customerSearch" v-model="form.business_name" @select='selectCustomer'>-->
            <!--<template slot-scope="props">-->
            <!--<div class="name">{{ props.item.value }}[{{props.item.id}}]</div>-->
            <!--</template>-->
            <!--</el-autocomplete>-->
            <el-select v-if="!isEdit" :disabled="isEdit" v-model="form.business_user_id" @change="selectCustomer" filterable
                       placeholder="请选择">
              <el-option
                v-for="item in tempCustomerInfo"
                :key="item.business_user_id"
                :label="item.value"
                :value="item.business_user_id">
              </el-option>
            </el-select>
            <div v-else>
              {{form.business_name}}
      ...
      <el-row>
        ...
        <el-col :span="8">
          <el-form-item size="mini" label="订单状态">
            {{form.order_status == null? '暂无': form.order_status}}
          </el-form-item>
        </el-col>
        <el-col :span="8">
      ...
      <el-row>
        ...
        <el-col :gutter="10" v-for="product in form.products" :span="6" style="border: 1px solid #eeeeee;padding: 10px">
          <el-form-item label-width="50px" size="mini" label="商品">
          ...
          <!--{{product.brand}}-->
          <!--</el-form-item>-->
          <el-form-item label-width="50px" size="mini" label="规格">
          ...
          <el-form-item label-width="50px" size="mini" label="毛重">
            {{product.gross_weight}}
          </el-form-item>
          <el-form-item label-width="50px" size="mini" label="数量">
            <el-input size="mini" placeholder="请输入数量" v-model="product.num"></el-input>
          </el-form-item>
          <el-form-item label-width="50px" size="mini" label="报价">
            {{product.original_price}}
          </el-form-item>
          <el-form-item label-width="50px" size="mini" label="单价">
    ...
    <el-dialog :center="true" :modal="false" title="添加商品" :visible.sync="on_off.isAddProduct" @close="addProductReset"
      ...
      <el-form>
        ...
        <el-form-item v-show="tempProduct.product_id" label-width="50px" size="mini" label="规格">
        ...
        <el-form-item v-show="tempProduct.product_standard_id" label-width="50px" size="mini" label="毛重">
          {{tempProduct.gross_weight}}
        </el-form-item>
        <el-form-item v-show="tempProduct.product_standard_id" label-width="50px" size="mini" label="报价">
          {{tempProduct.original_price}}
        </el-form-item>
        <el-form-item v-show="tempProduct.product_standard_id" label-width="50px" size="mini" label="单价">
...
<script>
  ...
  import {SHIPMENT_TYPE_LIST} from '../../../store/action-types'

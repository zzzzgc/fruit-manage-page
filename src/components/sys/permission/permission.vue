  ...
  <el-container style="height: 100%">
    <el-aside width="300px" style="">
      <div style="margin: 50px;">
        <el-tree :data="tempPermission" :props="defaultProps" @node-click="handleNodeClick">
    ...
    <el-main>
      ...
      <el-form ref="form" :model="form" label-position="top" label-width="80px">
        ...
        <el-form-item label="细分权限">
          <el-input size="mini" v-model="form.permission_key">
          </el-input>
        </el-form-item>
        <el-form-item label="权限名称">
          <el-input size="mini" v-model="form.permission_name"></el-input>
        </el-form-item>
        <el-form-item label="描述">
          <el-input size="mini" v-model="form.describe"></el-input>
        </el-form-item>
        <el-form-item>
...
<script>
  export default {
    ...
    methods: {
      init: function () {
        this.$http.post('/manage/permission/getAllPermission').then(
          (response) => {
      ...
      saveAndUpdate: function () {
        this.$http.post('/manage/permission/saveAndUpdate', this.form).then(
          (response) => {
      ...
      remove: function (node, data) {
        this.$confirm('此操作将删除该目录, 是否继续?', '提示', {
          ...
          if (data.id != null) {
            // 删除id指定的菜单 TODO
            this.$http.post('/manage/permission/remove', data).then(
              (response) => {
      ...
      append: function (data) {
        console.log(data)
        // 拦截三级节点
        if (data.permission_key.split(':')[1] !== '*') {
          this.warningMsg('不可以创建三级节点')
          return false
        }
        // 追加sonMenu
        ...
        let newPermission = {permission_key: data.permission_key.split(':')[0] + ':', permission_name: '', url: '/', describe: ''}

        this.$http.post('/manage/menu/saveAndUpdate', newPermission).then(
          (response) => {
            newPermission.id = response.data.id
            data.sonMenu.push(newPermission)
            this.handleNodeClick(newPermission)
            // console.log(document.getElementById(newMenu.id))
          }
        )
        // TODO 异常报错
        document.getElementById(newPermission.id).children.click()
      },

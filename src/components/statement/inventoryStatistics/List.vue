<template>
  <div class="table-list">
    <div class="action">
      <el-button type="primary" size="small" @click="exportInventoryStatistics">导出库存统计报表</el-button>
    </div>
...
<script>
  ...
  export default {
    name: 'inventory-statistics-list',
    components: {},
    created: function () {
      bus.$on(inventoryStatistics.search, (searchData) => { // 监听外部查询数据事件
    ...
    methods: {
      ...
      getData: function () { // 获取服务器数据
        this.$http.post('/manage/statement/inventoryStatistics/getData', Object.assign({}, this.searchData, this.pageInfo, this.planInfo)).then((response) => {
          this.pageInfo.totalRec = response.data.totalRow
          this.tableData = response.data.list
        })
      },
      exportInventoryStatistics: function () {
        this.$http.post('/manage/statement/inventoryStatistics/exportInventoryStatistics', Object.assign({}, this.searchData, this.pageInfo, this.planInfo)).then((response) => {

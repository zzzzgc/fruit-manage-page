<template>
  <div class="table-list">
    ...
    <el-table :data="tableData" @selection-change="selectChange" :default-sort="groupInfo" @sort-change="sortChange" border highlight-current-row style="width: 100%">
      ...
      <el-table-column prop="business_name" label="商户名称" width="200"></el-table-column>
      <el-table-column prop="pay_all_money" label="订单总货款" width="200"></el-table-column>
      <el-table-column prop="total_gross_margin" label="毛利总额" width="200"></el-table-column>
      <el-table-column label="毛利率" width="70">
        <template slot-scope="scope">
          {{Number(scope.row.gross_margin).toFixed(1)}}%
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  export default {
    components: {},
    created: function () {
      bus.$on(salesMargin.search, (searchData) => { // 监听外部查询数据事件
    ...
    methods: {
      ...
      getData: function () { // 获取服务器数据
        this.$http.post('/manage/statement/salesMargin/getData', Object.assign({}, this.searchData, this.pageInfo, this.groupInfo)).then((response) => {
          this.pageInfo.totalRec = response.data.totalRow
          this.tableData = response.data.list
          this.$http.post('/manage/statement/salesMargin/getTotalInfo', Object.assign({}, this.searchData, this.pageInfo, this.groupInfo)).then(
            (response) => {
      ...
      inputExcel: function () {  // 导出订单数据
        this.$http.post('/manage/statement/salesMargin/exportExcel', Object.assign({}, this.searchData)).then((response) => {

  ...
  <div class="table-list">
    <div class="action">
      ...
      <div style="display: inline-block">
        <span>汇总欠款:{{this.totalData.arrearageCount}} </span>
      </div>
      <!--{{this.tableData}}-->
    </div>
    <el-table :data="tableData" @selection-change="selectChange" :default-sort="groupInfo" @sort-change="sortChange" border highlight-current-row style="width: 100%">
      ...
      <el-table-column prop="business_name" label="商家名称" width="200"></el-table-column>
      <el-table-column prop="order_id" label="订单编号" width="200"></el-table-column>
      <el-table-column prop="nick_name" label="销售名称" width="200"></el-table-column>
      <el-table-column prop="arrearage" label="订单欠款" width="200"></el-table-column>
      <el-table-column prop="arrearage_total" label="总欠款" width="200"></el-table-column>
...
<script>
  import bus, {arrearsStatistics} from '../../../common/bus.js'
  import {localStorageKeys} from '../../../common/const.js'

  export default {
    components: {},
    created: function () {
      bus.$on(arrearsStatistics.search, (searchData) => { // 监听外部查询数据事件
    ...
    methods: {
      ...
      getData: function () { // 获取服务器数据
        this.$http.post('/manage/statement/arrearsStatistics/getData', Object.assign({}, this.searchData, this.pageInfo, this.groupInfo)).then(
          (response) => {
            this.pageInfo.totalRec = response.data['pageData'].totalRow
            this.tableData = response.data['pageData'].list
            this.totalData.arrearageCount = response.data['arrearageCount']
            // this.pageInfo.totalRec = response.data.totalRow
            // this.tableData = response.data.list
            // this.$http.post('/manage/statement/arrearsStatistics/getTotalInfo', Object.assign({}, this.searchData, this.pageInfo, this.groupInfo)).then(
            //   (response) => {
      ...
      inputExcel: function () {  // 导出订单数据
        this.$http.post('/manage/statement/arrearsStatistics/exportExcel', Object.assign({}, this.searchData)).then((resposne) => {
          location.href = this._downloadFilePath + resposne.data[0]
        })
        // location.href = ajaxPrefix + '/manage/statement/arrearsStatistics/exportExcel?order_cycle_date=' + this.searchData.order_cycle_date + '&nick_name=' + this.searchData.nick_name + '&business_name=' + this.searchData.business_name
      },
